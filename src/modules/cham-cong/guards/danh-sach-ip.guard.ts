 ...
 */
@Injectable()
export class DanhSachIpGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (whitelist.length === 0) {
      // Nếu whitelist rỗng và đã bật, từ chối tất cả
   ...
   * Hỗ trợ các header proxy phổ biến
   */
  ...
  /**
   * Kiểm tra xem IP có phải localhost không
   */
   ...
   * Hỗ trợ: IP đơn lẻ hoặc CIDR notation (ví dụ: 192.168.1.0/24)
   */
  private isIpMatch(clientIp: string, pattern: string): boolean {
    ...
    // Kiểm tra wildcard (ví dụ: 192.168.1.*)
    if (normalizedPattern.includes('*')) {
  ...
  /**
   * Kiểm tra IP có thuộc CIDR range không
   */
